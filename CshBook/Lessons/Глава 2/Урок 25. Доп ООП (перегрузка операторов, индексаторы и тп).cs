...
namespace CshBook.Lessons
{
    ...
    */
    ...
    /*
        ...
        # Реализуйте класс CustomDictionary с индексатором, который будет позволять получать доступ к элементам
          по строковому ключу (как в обычном словаре). Добавьте второй индексатор, который будет позволять
          получать доступ к элементам по индексу (порядковому номеру добавления).
